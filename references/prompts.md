...
本文件包含多模型模式下各阶段使用的 subagent prompt 模板。执行时读取本文件获取模板，填充实际内容后传入 subagent。

## 独立评估 Prompt（策略 A 第四步-A）
...
{列出 Skill 的附带资源及其摘要，如没有则写"无附带资源"}

## 评分标准

按以下 8 个维度打分，每个维度 0-10 分：

{读取 references/dimensions.md 的 D1-D8 完整内容（含评分标准表）粘贴于此}

## 评分计算

{读取 references/dimensions.md「评分计算」章节的加权公式和等级映射表粘贴于此}
...
**语言适配**：若目标 Skill 为英文，subagent prompt 中的指令和输出格式也应为英文。

**结果标记**：收集每个 subagent 的返回结果，标记其对应的模型名称（如 "Sonnet 评估"、"GLM-5 评估"）。

## 交叉互审 Prompt（策略 A 第五步-A）
